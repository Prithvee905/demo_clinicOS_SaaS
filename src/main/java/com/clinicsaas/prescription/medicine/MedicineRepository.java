...
public interface MedicineRepository extends JpaRepository<Medicine, UUID> {
    ...
    @Query(value = "SELECT * FROM medicines " +
                   "WHERE clinic_id = :clinicId AND status = 'ACTIVE' AND " +
                   "(medicine_name ILIKE CONCAT('%', :query, '%') OR similarity(medicine_name, :query) > 0.1) " +
                   "ORDER BY similarity(medicine_name, :query) DESC LIMIT 15", nativeQuery = true)
    ...
    @Query("SELECT m FROM Medicine m WHERE m.clinicId = :clinicId AND m.status = 'ACTIVE' AND " +
           "(:search IS NULL OR LOWER(m.medicineName) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(m.medicineCode) LIKE LOWER(CONCAT('%', :search, '%')))")
